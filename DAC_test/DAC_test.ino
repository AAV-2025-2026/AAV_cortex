...
// ============================================================
// Pin / Channel definitions — from hardware_config.h
// ============================================================
...
// ============================================================
// Globals — ws must be declared before WSSerial
// ============================================================
WebServer        server(80);
...
// ============================================================
// WSSerial — mirrors every USER_SERIAL print to WebSocket
// ============================================================
...
WSSerial WSerial;
#define USER_SERIAL WSerial   // all USER_SERIAL calls → Serial + WebSocket

// ============================================================
// DAC — from MotorCtrl.cpp
// ============================================================
...
// ============================================================
// PCA9555 external GPIO — from hardware_fns.cpp
// ============================================================
...
void publishStatus() {
    char buf[64];
    snprintf(buf, sizeof(buf), "STATUS:%.2f,%.2f,%.2f",
             current_steering, current_speed, current_accel);
    USER_SERIAL.println(buf);
}

// ============================================================
// HTTP status page with live WebSocket log
// ============================================================
void handleStatus() {
  ...
  <script>
    ...
    ws.onmessage = function(e) {
      ...
      if (e.data.startsWith('STATUS:')) {
        var parts = e.data.replace('STATUS:','').split(',');
...
//===============================
void handleScan() {
    Wire.begin(I2C_SDA, I2C_SCL);   // ← reinit before scan
    delay(50);
    ...
    for (uint8_t addr = 1; addr < 127; addr++) {
        ...
        if (Wire.endTransmission() == 0) {
            ...
            snprintf(line, sizeof(line), "  Found: 0x%02X%s%s%s%s", addr,
                ...
                addr == 0x04 ? " <- JRK brake"   : "",
                addr == 0x20 ? " <- PCA9555"      : "",
                addr == 0x0C ? " <- DAC (A0=GND)" : "");
            USER_SERIAL.println(line);
            result += String(line) + "\n";
        }
    }
    if (!found) {
        USER_SERIAL.println("  No I2C devices found!");
...
void setup() {
    ...
    // Buffer early messages before WebSocket is ready
    String earlyLog = "";
    earlyLog += "\n=== ESP32: ROS2 + iBUS RC Override + DAC + OTA + WS ===\n";

    // LEDs — builtin ON at boot
    ...
    if (WiFi.status() == WL_CONNECTED) {
        ...
        server.on("/scan",   HTTP_GET, handleScan);     // ← I2C scan on demand
        ...
        Serial.printf("WS Log -> ws://%s:81/\n",       WiFi.localIP().toString().c_str());
        digitalWrite(GREEN_LED_PIN, HIGH);
    } else {
        earlyLog += "\nWiFi failed — continuing without OTA/WS";
        Serial.println("\nWiFi failed — continuing without OTA/WS");
        digitalWrite(RED_LED_PIN, HIGH);
    }

    // Flush buffered early log to WebSocket
    int start = 0;
    ...
    while (!Wire.begin(I2C_SDA, I2C_SCL)) {
        ...
        if (millis() - i2c_start > 3000) {
            USER_SERIAL.println("WARNING: I2C failed after 3s — continuing");
            break;
        }
    }
    USER_SERIAL.printf("I2C -> SDA:%d SCL:%d\n", I2C_SDA, I2C_SCL);

    // PCA9555 external GPIO expander
    ioex.attach(Wire);
    ...
    if (dac_ready) {
        ...
        USER_SERIAL.println("WARNING: DAC not found — continuing without throttle");
        digitalWrite(RED_LED_PIN, HIGH);
    }

    // JRK G2 — center on boot
    jrk_steer.setTarget(2048);
    USER_SERIAL.println("JRK steer -> centered (2048)");

    USER_SERIAL.println("Ready — SwA: RC_EN | SwC: TOP=Brake, MID=Fwd, LOW=Rev");
    USER_SERIAL.printf("Scan   -> http://%s/scan\n", WiFi.localIP().toString().c_str());
}
...
void loop() {
    ...
    if (rc_mode) {
        ...
        if (throttle_rc    > 100 && throttle_rc    < 2200 &&
            ...
            int drive_mode = 0;
            if (drive_mode_raw < 1200)      drive_mode = 1;
            else if (drive_mode_raw > 1800) drive_mode = 2;

            current_steering = map(steering_rc, 1000, 2000, -100, 100) / 100.0;
            float throttle   = map(throttle_rc, 1000, 2000, -100, 100) / 100.0;

            // Steering → JRK G2
            uint16_t jrk_target = (uint16_t)map(steering_rc, 1000, 2000, 4095, 0);
            jrk_steer.setTarget(constrain(jrk_target, 0, 4095));
            ...
            publishStatus();
            USER_SERIAL.println("RC: No valid signal - STOPPED");
        }
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (idx >= 13) {
                    ...
                    if (calc == checksum) {
                        ...
                        uint16_t jrk_target = (uint16_t)map(
                            (long)(steering * 100), -100, 100, 4095, 0);
                        jrk_steer.setTarget(constrain(jrk_target, 0, 4095));

                        USER_SERIAL.printf("ROS: steer=%.2f speed=%.2f DAC=%d\n",
                                           steering, speed, dac_val);
                        publishStatus();
                        idx = 0;
                    } else {
